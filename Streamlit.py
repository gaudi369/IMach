...
st.title("Aerodynamics Analysis Tool")
...
def draw_shape(points):
    ...
    xs, ys = zip(*points)
    xs += (xs[0],)  # Close the shape by connecting the last point to the first
    ys += (ys[0],)
    ...
    ax.set_title(f"Polygon with {len(points)} Points")

    # Display the plot using Streamlit's function
    st.pyplot(fig)
...
# App main interface
st.title("Draw a Geometric Shape")
...
def update_points(button):
  ...
  # Plotting
def plot_flow_properties(flow_props):
    mach_numbers = [fp[0] for fp in flow_props]
    pressures = [fp[1] for fp in flow_props]
    fig, axs = plt.subplots(2, 1, figsize=(10, 8))
    axs[0].plot(mach_numbers, marker='o', linestyle='-')
    axs[0].set_title('Mach Numbers')
    axs[0].set_xlabel('Point Number')
    axs[0].set_ylabel('Mach Number')
    axs[1].plot(pressures, marker='s', linestyle='-')
